...
| VpcCIDR             | VPC CIDR allowed to send telemetry; required when telemetry ingress is enabled.        |           String            |                                                                                     |                |
...
With a compatible Nuon control plane and a VPC template exposing `VpcCIDR`, the parent install stack exposes
`EnableTelemetryIngress` under Runner Configuration and supplies the VPC parameters automatically. The endpoint is
...
The internal NLB uses the runner's private subnet and accepts port 4318 from the VPC CIDR. Its DNS name remains stable
when the ASG replaces the runner. An empty output means no endpoint is provisioned; only configure producers when an
endpoint is present. Existing pinned templates must be upgraded to a release containing telemetry ingress support.
